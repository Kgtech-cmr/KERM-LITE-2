...
System({
	...
	const ping = await message.send("*📊running🀄️*");
...
System({
   ...
   if (!message.reply_message.viewones) return await message.reply("_*Répondre à une vue unique*_");
...
System({
   ...
   type: "user",
    ...
    await message.reply('_Redémarrage..._')
...
async (message, match) => {
    ...
    if (match && message.sudo.includes(message.sender)) {
        if (await setData(message.user.id, match, "true", "alive"))
            return await message.send('_Mise à jour du message Alive_');
        else
            return await message.send('_Erreur lors de la mise à jour_');
    }
...
System({
    ...
    if (!match) return m.reply("*Besoin de texte*\n_Exemple : .readmore Salut\\comment allez-vous_");
...
System({
    ...
    if (!match) return await message.reply("*Besoin d'un nom de lieu pour connaître l'heure*\n_Exemple : .temps Cameroun_");
    var p = match.toLowerCase();
    const res = await fetch(IronMan(`ironman/search/time?loc=${p}`));
    const data = await res.json();
    if (data.error === 'no place') return await message.send("_*Aucun endroit trouvé*_");
    ...
    let msg = `*Hᴇᴜʀᴇ Aᴄᴛᴜᴇʟʟᴇ*\n(12-hour format): ${time12WithMs}\n(24-hour format): ${time24WithMs}\n`;
    msg += `*ʟᴏᴄᴀᴛɪᴏɴ:* ${name}\n`;
    if (state) {
        msg += `*Éᴛᴀᴛ:* ${state}\n`;
    }
    msg += `*ᴄᴀᴘɪᴛᴀʟᴇ:* ${capital}\n`;
    msg += `*Mᴏɴɴᴀɪᴇ:* ${currName} (${currCode})\n`;
    msg += `*Cᴏᴅᴇ ᴅᴜ ᴘᴀʏs:* +${phone}`;
...
System({
       ...
       if(!m.quoted) return m.reply("_Répondre à un message_");
      const msg = await m.store.loadMessage(m.jid, m.reply_message.id);
      if(!msg) return m.reply("_Message introuvable du magasin_");
...
System({
    ...
    if (!match) {
        await message.reply("*EXEMPLE* *:* _.calc 2+2_");
        return;
    }
